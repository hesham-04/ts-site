...
export const DocumentList = ({parentId, level=0, }:DocumentListProps) => {
    ...
    return (
        <>
            <p style={{paddingLeft: level ? `${(level * 12) + 25}px` : '12px'}} className={cn('hidden text-sm  font-medium text-muted-foreground/80 mb-1', expanded && 'last:block', level=== 0 && 'hidden')}>
                No Pages inside
            </p>
